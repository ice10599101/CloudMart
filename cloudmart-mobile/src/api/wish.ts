...
/** 等级提升事件（签到瞬间检测；未提升为 null，庆祝弹窗/APP 本地推送依据） */
...
export const wishApi = {
    ...
    // ---- 每日签到（文档 2.6 / L1915：签到 +5 星光 + 等级提升事件）----
    dailySignin: () =>
    ...
    // ---- 心愿打卡日历（Sprint 1.3 验收：按心愿当月已打卡日期）----
    ...
    /** 捞瓶（null = 海里无瓶） */
    fishDriftBottle: () =>
        request<DriftBottleItem | null>({ url: '/wish/drift-bottles/fish', method: 'POST' }),
    /** 我的漂流瓶（我投出的 + 我捞到的，id 倒序） */
    listMyDriftBottles: () => request<DriftBottleItem[]>({ url: '/wish/drift-bottles/mine' }),
    /** 瓶互动（BLESS / LIGHT；仅捞起人、仅关联心愿瓶；当日重复 WISH_RATE_LIMITED） */
    interactDriftBottle: (bottleId: number | string, type: 'BLESS' | 'LIGHT') =>
        request<DriftBottleItem>({
            url: `/wish/drift-bottles/${bottleId}/interactions`,
            method: 'POST',
            data: { type },
        }),
    /** 瓶评论列表（cursor 分页；非投瓶人/捞起人 404） */
    ...
    /** 发表瓶评论（parentId = 回复目标评论 ID；isAnonymous 缺省/true 匿名） */
    ...
    // ---- 虚拟工坊 + 收藏馆 + 品牌许愿池（Sprint 3.6，四AC R1）----
