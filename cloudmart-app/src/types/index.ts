...
export interface Post {
  ...
  coverImage?: string
  mediaUrls?: string[]
  /** 关联好物商品 ID（好物帖；null=普通帖，对齐 Web 端契约） */
...
export interface Notification {
  id: number
  /** 后端 NotificationDTO.type 实为字符串（LIKE/COMMENT/CHECKIN_REMINDER 等）；数字为平台旧通知遗留 */
  type: number | string
  title: string
  content: string
  isRead: boolean
  createdAt: string
  sender?: UserBasic
  /** 关联业务 ID（心愿域通知为心愿/胶囊 ID） */
  bizId?: number | null
  /** 关联业务类型（EXPECTED_MANAGEMENT/COMPANION_REMINDER/WISH/CAPSULE/POST/USER 等） */
  bizType?: string | null
  /** 操作者用户 ID（谁做的互动，前端据此跳转其主页/回关） */
  actorId?: number | null
}
...
/** 心愿互动列表项（祝福墙展示；userId 为 null 表示匿名） */
...
// ---- 还愿（Sprint 1.10，与 mall-wish FulfillmentSubmitVO/FulfillmentDetailVO 对齐）----
...
// ==== 漂流瓶配额（投瓶 10 个/天、打捞 20 次/天，UTC 自然日） ====
...
/** 地图前端配置（契约对齐 mall-wish GET /wish/map/config：nacos amap.* 由后端下发） */
