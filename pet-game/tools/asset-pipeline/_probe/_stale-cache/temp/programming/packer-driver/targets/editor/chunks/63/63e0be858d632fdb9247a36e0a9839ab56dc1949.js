System.register(["__unresolved_0", "cc", "__unresolved_1"], function (_export, _context) {
  ...
  function buildWalls(parent, kit) {
    ...
    const wall = style(C.wall, {
      shade: C.wallShade
    });
    const base = style(C.baseboard, {
  ...
  function buildWindow(parent, kit) {
    ...
    const frame = style(C.windowFrame, {
  ...
  function buildWallDecor(parent, kit) {
    const root = kit.make3dNode(parent, 'WallDecor', new Vec3(0, 0, 0)); // 挂画
    ...
    const books = [[C.book1, -0.5, 0.16], [C.book2, -0.3, 0.14], [C.book3, -0.11, 0.18]];
  ...
  function buildCabinet(parent, kit) {
    ...
    const drawerLine = style((_crd && shift === void 0 ? (_reportPossibleCrUseOfshift({
      error: Error()
  ...
  function buildPlant(parent, kit) {
    ...
    const leaves = [[0, 1.45, 0, 0.34], [0.3, 1.25, 0.1, 0.28], [-0.3, 1.22, 0.08, 0.3], [0.16, 1.62, -0.08, 0.24], [-0.18, 1.58, -0.06, 0.22]];
  ...
  function buildToys(parent, kit) {
    const root = kit.make3dNode(parent, 'Toys', new Vec3(0, 0, 0)); // 皮球（带白色条纹感：叠加一圈小球）
  ...
  function buildOrbs(parent, kit) {
    ...
    const orbs = [];
    const seeds = [[-2.4, 0.9, -1.2, 0.05], [1.7, 1.3, -0.6, 0.042], [-0.9, 1.9, -1.8, 0.048], [2.6, 0.7, 0.6, 0.038], [-2.9, 1.7, 0.9, 0.045], [0.6, 2.3, -1.4, 0.04], [3.1, 1.8, -2.0, 0.05], [-1.8, 0.6, 1.9, 0.036]];
  ...
  function buildContactShadows(parent, kit) {
    ...
    const blob = (name, rx, rz, x, y, z, alpha) => {
