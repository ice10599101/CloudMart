  ...
  return {
    ...
    execute: function () {
      ...
      _export("PetAnimations", PetAnimations = class PetAnimations {
        ...
        static breathe(rig, t) {
          var rate = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 2.05;
          var wave = Math.sin(t * rate);
          var squeeze = wave * 0.022;
        ...
        static tailSway(rig, t, mood) {
          var speed = 1.9 + mood * 3.4;
          var amplitude = 9 + mood * 15;
          var base = rig.tailBaseRot;
        ...
        static earPose(rig, t, droop) {
          var twitchGate = Math.sin(t * 5.7 + 1.3) > 0.94 ? Math.sin(t * 27) * 6 : 0;
          rig.ears.forEach((ear, index) => {
            var base = rig.earBase[index];
        ...
        static look(rig, x, y) {
          ...
          for (var _pupil of rig.pupils) {
            _pupil.setPosition(base.x + x * 0.032, base.y + y * 0.030, base.z);
          }
        }
        ...
        static blink(rig, openness) {
          var scaleY = Math.max(0.06, Math.min(1.14, openness * 1.14));

          for (var eye of rig.eyeGroups) {
        ...
        static setBlush(rig, strength) {
          var s = Math.max(0.5, Math.min(1.45, strength));
          ...
          var base = rig.blushBase || BLUSH_BASE_SCALE;

          for (var blush of rig.blush) {
        ...
        static resetPose(rig) {
          for (var node of [rig.root, rig.body, rig.head, rig.tail, rig.tuft]) {
