  ...
  return {
    ...
    execute: function () {
      ...
      _export("PetHud", PetHud = class PetHud {
        ...
        showBubble(text) {
          var duration = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 3.2;
        ...
        setBusy(intent) {
          for (var [key, node] of this.buttons) {
            var opacity = node.getComponent(UIOpacity);
            ...
            var busy = intent !== null && key === intent;
            Tween.stopAllByTarget(node);
            Tween.stopAllByTarget(opacity);
        ...
        update(dt) {
          ...
          for (var row of this.bars.values()) {
        ...
        buildNamePlate() {
          var top = this.height / 2 - 46;
          var plate = this.node('hud-nameplate', 360, 64, 0, top);
          var g = plate.addComponent(Graphics);
          ...
          var badge = this.node('hud-badge', 52, 52, -142, 0);
          var bg = badge.addComponent(Graphics);
        ...
        buildStateCard() {
          var cardW = 286;
          var cardH = 232;
          var cx = -this.width / 2 + cardW / 2 + 18;
          var cy = this.height / 2 - cardH / 2 - 90;
          var card = this.node('hud-state-card', cardW, cardH, cx, cy);
          ...
          for (var row of _crd && STATE_ROWS === void 0 ? (_reportPossibleCrUseOfSTATE_ROWS({
            error: Error()
          }), STATE_ROWS) : STATE_ROWS) {
            var rowNode = this.node('hud-row-' + row.key, cardW, 30, 0, rowY);
            ...
            var track = this.node('track', barWidth, 13, 22, 0);
            var trackG = track.addComponent(Graphics);
            ...
            var fillNode = this.node('fill', barWidth, 13, 22, 0);
            var fillG = fillNode.addComponent(Graphics);
            rowNode.addChild(fillNode);
            var value = this.label(rowNode, '0', 15, cardW / 2 - 26, 0, (_crd && HUD === void 0 ? (_reportPossibleCrUseOfHUD({
              error: Error()
            }), HUD) : HUD).textMain);
          ...
          var expLabel = this.label(card, '⭐ 成长', 15, -cardW / 2 + 48, rowY + 4, (_crd && HUD === void 0 ? (_reportPossibleCrUseOfHUD({
            error: Error()
          }), HUD) : HUD).textSub);
          expLabel.horizontalAlign = Label.HorizontalAlign.LEFT;
          var expTrack = this.node('hud-exp-track', cardW - 44, 12, 0, rowY - 22);
          var expTrackG = expTrack.addComponent(Graphics);
          ...
          var expFillNode = this.node('hud-exp-fill', cardW - 44, 12, 0, rowY - 22);
        ...
        buildActionButtons() {
          var y = -this.height / 2 + 118;
          var gap = 128;
          (_crd && ACTION_BUTTONS === void 0 ? (_reportPossibleCrUseOfACTION_BUTTONS({
            error: Error()
          }), ACTION_BUTTONS) : ACTION_BUTTONS).forEach((action, index) => {
            var x = (index - ((_crd && ACTION_BUTTONS === void 0 ? (_reportPossibleCrUseOfACTION_BUTTONS({
              error: Error()
            }), ACTION_BUTTONS) : ACTION_BUTTONS).length - 1) / 2) * gap;
            var node = this.node('hud-btn-' + action.intent, 84, 84, x, y);
            var g = node.addComponent(Graphics); // 圆形底座 + 底部阴影 + 顶部高光（立体糖果感）
        ...
        buildNavButtons() {
          var y = -this.height / 2 + 42;
          var gap = 116;
          (_crd && NAV_BUTTONS === void 0 ? (_reportPossibleCrUseOfNAV_BUTTONS({
            error: Error()
          }), NAV_BUTTONS) : NAV_BUTTONS).forEach((nav, index) => {
            var x = (index - ((_crd && NAV_BUTTONS === void 0 ? (_reportPossibleCrUseOfNAV_BUTTONS({
              error: Error()
            }), NAV_BUTTONS) : NAV_BUTTONS).length - 1) / 2) * gap;
            var node = this.node('hud-nav-' + nav.intent, 108, 42, x, y);
        ...
        buildBubble() {
          ...
          var node = this.node('hud-bubble', 340, 74, 0, this.height / 2 - 93);
          var g = node.addComponent(Graphics);
        ...
        paintBar(row) {
          var g = row.fill;
          g.clear();
          var width = Math.max(0, row.display) * row.width;
          ...
          var value = row.value;
          var display = String(Math.round(row.display * 100));
        ...
        paintExp() {
          ...
          var track = 286 - 44 - 4;
          var width = Math.max(0, this.expDisplay) * track;
        ...
        label(parent, text, fontSize, x, y, color) {
          var width = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : 0;
          var node = this.node('label', width > 0 ? width : 10, Math.round(fontSize * 1.4), x, y);
