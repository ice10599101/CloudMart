System.register(["__unresolved_0", "cc", "__unresolved_1", "__unresolved_2"], function (_export, _context) {
  ...
  function buildCatPet(petParent, groundParent, kit, palette, accessoryKey, buildAccessory) {
    const p = palette; // ---- 材质风格（毛绒物种：高绒光 + 弱而宽的高光；暗部为暖灰紫，不发黑） ----
    ...
    const head = kit.make3dNode(body, 'Head', new Vec3(0, SIZE.headY - SIZE.bodyY, SIZE.headZOffset + 0.02), {
      scale: new Vec3(HEAD_SCALE, HEAD_SCALE, HEAD_SCALE)
    });
    const blushColor = new Color().fromHEX('#F3A7AE'); // 腮红位置换算成头部 uv：u = 方位角 / 2π，v = 归一化高度（与 lathe 写入的 uv 一致）

    const blushSurfaceZ = (_crd && frontZ === void 0 ? (_reportPossibleCrUseOffrontZ({
      error: Error()
    ...
    const skull = kit.surf(head, 'Skull', (_crd && transformed === void 0 ? (_reportPossibleCrUseOftransformed({
      error: Error()
    }), transformed) : transformed)((_crd && lathe === void 0 ? (_reportPossibleCrUseOflathe({
      error: Error()
    ...
    const earInfo = buildEars(head, kit, main); // ---- 面部 ----

    const face = buildFace(head, kit, p, soft); // ---- 呆毛（单撮）：短、粗、向前上方弯成一道干净曲线。
    ...
    const accessory = buildAccessory(petParent, accessoryKey, SIZE.collarY); // ---- 地面软影（挂房间层：宠物跳起时影子留在地面，脚底不悬浮） ----
    ...
    const shadowCenter = new Vec3(petParent.position.x, 0.082, petParent.position.z - 0.02); // 站姿的投影是**沿身体长轴拉长的椭圆**（不是正圆盘）：与四足支撑面的形状一致

    const shadowLayers = [[0.36, 0.013, 0.46, 0.06], [0.27, 0.012, 0.35, 0.15], [0.18, 0.011, 0.24, 0.26]];
    const shadowNodes = shadowLayers.map(([rx, ry, rz, darkening]) => {
      const tone = (_crd && shift === void 0 ? (_reportPossibleCrUseOfshift({
        error: Error()
  ...
  function buildLegs(root, kit, main, pawStyle) {
    const legGeo = (_crd && lathe === void 0 ? (_reportPossibleCrUseOflathe({
      error: Error()
    ...
    const limbStyle = { ...main,
      outline: 0
    };
    const legGeos = [];
    const toeGeos = [];
    const legs = [[SIZE.frontLegX, SIZE.frontLegZ, 1], [-SIZE.frontLegX, SIZE.frontLegZ, -1], [SIZE.backLegX, SIZE.backLegZ, 1], [-SIZE.backLegX, SIZE.backLegZ, -1]];
  ...
  function buildEars(head, kit, main) {
    ...
    for (const side of [-1, 1]) {
      ...
      const asymY = side === -1 ? 2 : 0; // 耳根埋在头颅上侧内部（保证无接缝）；外倾 20° 让耳朵从正面看来是"立起来的三角"

      const root = kit.make3dNode(head, 'Ear', new Vec3(SIZE.earX * side, SIZE.earY, 0.012), {
  ...
  function buildFace(head, kit, p, soft) {
    ...
    const eyeRadii = [SIZE.eyeRX, SIZE.eyeRY, SIZE.eyeRZ];
    const eyeZ = (_crd && frontZ === void 0 ? (_reportPossibleCrUseOffrontZ({
      error: Error()
    ...
    for (const side of [-1, 1]) {
      const group = kit.make3dNode(head, 'Eye', new Vec3(SIZE.eyeX * side, SIZE.eyeY, eyeZ), {
    ...
    const muzzleZ = (_crd && frontZ === void 0 ? (_reportPossibleCrUseOffrontZ({
      error: Error()
    ...
    kit.surf(head, 'Muzzle', (_crd && ellipsoid === void 0 ? (_reportPossibleCrUseOfellipsoid({
      error: Error()
    }), ellipsoid) : ellipsoid)(MUZZLE[0], MUZZLE[1], MUZZLE[2], 24, 16), { ...soft,
    ...
    const noseZ = (_crd && frontZ === void 0 ? (_reportPossibleCrUseOffrontZ({
      error: Error()
    ...
    const mouthColor = (_crd && shift === void 0 ? (_reportPossibleCrUseOfshift({
      error: Error()
    ...
    const arcNodes = (halfWidth, lift, radius, yBase) => {
      const nodes = [];
    ...
    const open = kit.make3dNode(head, 'Open', new Vec3(0, SIZE.mouthY + 0.004, mouthSurfaceZ(0, SIZE.mouthY) - 0.004));
    open.active = false;
    ...
    const blush = [kit.make3dNode(head, 'BlushL', new Vec3(-SIZE.blushX, SIZE.blushY, 0)), kit.make3dNode(head, 'BlushR', new Vec3(SIZE.blushX, SIZE.blushY, 0))];
    return {
      eyeGroups,
      pupils,
      mouth: {
