      ...
      _export("PetGameRoot", PetGameRoot = (_dec = ccclass('PetGameRoot'), _dec(_class = class PetGameRoot extends Component {
          ...
          resources.load('effects/pet-toon', EffectAsset, (error, asset) => {
            ...
            resources.load(PET_ALIVE_TEX, Texture2D, (texError, tex) => {
              if (texError || !tex) {
                ...
                console.log("[pet-probe] alive texture ready: ".concat(tex.name));
              }
        ...
        loadPet() {
          resources.load(PET_MODEL_PATH, Prefab, (error, prefab) => {
            ...
            var clipPaths = PET_CLIPS.map(name => "".concat(PET_MODEL_DIR, "/").concat(name));
            var loaded = [];
          ...
          for (var c of node.components) {
            ...
            if (model.material && model.material.effectAsset) {
              ...
              if (get) {
                ...
                var mScale = get.call(model.material, 'albedoScale');
                extra += " tex=".concat(tex ? tex.name : 'null');
                extra += " albedo=".concat(albedo ? JSON.stringify(albedo) : 'null');
                extra += " aScale=".concat(mScale ? JSON.stringify(mScale) : 'null');
                extra += " passes=".concat((_passes = model.material.passes) === null || _passes === void 0 ? void 0 : _passes.length);
              }
            }
        ...
        configurePetMaterials(node) {
          var visit = n => {
            for (var comp of n.components) {
              ...
              var albedo = (_this$aliveTexture = this.aliveTexture) !== null && _this$aliveTexture !== void 0 ? _this$aliveTexture : embedded;
              ...
              var ctrlBack = toon.getProperty('mapCtrl');
              var lightBack = toon.getProperty('lightCtrl');
              console.log("[pet-probe] pet material ".concat(n.name, ": pet-toon albedoTex=") + "".concat(texBack ? texBack.uuid : 'null', " ") + "mapCtrl=".concat(ctrlBack ? "".concat(ctrlBack.x, ",").concat(ctrlBack.y, ",").concat(ctrlBack.z, ",").concat(ctrlBack.w) : 'null', " ") + "lightCtrl=".concat(lightBack ? "".concat(lightBack.x, ",").concat(lightBack.y) : 'null'));
            }
        ...
        probeFraming() {
          this.scheduleOnce(() => {
            ...
            var top = camera.worldToScreen(new Vec3(PET_POS.x, PET_HEIGHT, PET_POS.z), new Vec3());
        ...
        buildLighting(scene) {
          ...
          a.skyIllumHDR = 1400;
          console.log("[pet-probe] lighting: key=".concat(keyLight.illuminance, " fill=").concat(fillLight.illuminance, " ") + "ambLDR=".concat(ambient.skyIllum, " ambHDR=").concat(a.skyIllumHDR));
        }
        /**
         * 宠物脚下的接触阴影。
         *
