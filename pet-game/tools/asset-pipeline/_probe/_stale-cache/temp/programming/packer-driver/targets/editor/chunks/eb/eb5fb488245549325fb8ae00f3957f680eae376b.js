    ...
    execute: function () {
      ...
      _export("PetBuilderKit", PetBuilderKit = class PetBuilderKit {
        ...
        attach(node, geometry, style) {
          const model = node.addComponent(this.cc.ModelComponent);
          model.mesh = this.cc.utils.createMesh(geometry);
          const transparent = style.blend === true || style.alpha !== undefined && style.alpha < 255; // plain：强制引擎内置材质（标准透明路径）；其余部件优先 pet-toon。
          // 注意：必须以**最终选中的 effect** 判断技术索引 —— 若用"解析结果是否为空"判断，
          // 一旦出现"解析失败但兜底拿到 pet-toon"的组合，technique 索引就会越界，
          // 材质初始化静默失败并退回内置材质（曾导致所有 toon 表现失效）。

          const effect = style.plain ? this.pickFallbackEffect(model) : this.resolveToonEffect() || this.pickFallbackEffect(model);
          ...
          const color = style.alpha === undefined || style.alpha >= 255 ? style.color : new Color(style.color.r, style.color.g, style.color.b, style.alpha);
          const shade = style.glow ? new Color(color.r, color.g, color.b, color.a) : style.shade || (_crd && shift === void 0 ? (_reportPossibleCrUseOfshift({
            error: Error()
          ...
          try {
            ...
            if (useToon) {
              ...
              const edge = (_crd && shift === void 0 ? (_reportPossibleCrUseOfshift({
                error: Error()
        ...
        patch(parent, name, dir, angleU, angleV, radii, style, pos, inflate = 1.006) {
          const geometry = (_crd && spherePatch === void 0 ? (_reportPossibleCrUseOfspherePatch({
            error: Error()
          }), spherePatch) : spherePatch)(dir, angleU, angleV, [radii[0] * inflate, radii[1] * inflate, radii[2] * inflate]);
          return this.surf(parent, name, geometry, { ...style,
            outline: 0,
            fur: 0,
            spec: 0
          }, pos);
        ...
        setPartProperty(node, name, value) {
          const model = node.components.find(c => c.mesh !== undefined);
