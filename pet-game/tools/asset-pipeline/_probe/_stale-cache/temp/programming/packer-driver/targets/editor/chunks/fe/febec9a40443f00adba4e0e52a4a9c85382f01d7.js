System.register(["cc"], function (_export, _context) {
  ...
  function lathe(profile, segments = 32) {
    ...
    const normals = [];
    const uvs = [];
    const indices = []; // 预先求每层轮廓的 2D 法线：轮廓切线顺时针旋转 90° 指向外侧
  ...
  function ellipsoid(rx, ry, rz, segments = 32, rings = 22) {
    const positions = [];
    const normals = [];
    const uvs = [];
    const indices = [];
   ...
   */
  function sweep(nodes, radial = 18, capSteps = 3) {
    const count = nodes.length;
    const positions = [];
    const normals = [];
    const uvs = [];
    const indices = []; // 每个节点的切线
    ...
    const ref = Math.abs(tangents[0][1]) > 0.92 ? [1, 0, 0] : [0, 1, 0];
    ...
    for (let i = 0; i < count; i++) {
      ...
      let n = norm([normal[0] - t[0] * dot, normal[1] - t[1] * dot, normal[2] - t[2] * dot]);
    ...
    if (capSteps > 0 && count >= 2) {
      const startCap = [];
      const head = rings[0];
      const dirStart = [-tangents[0][0], -tangents[0][1], -tangents[0][2]];
    ...
    for (let i = 0; i < total; i++) {
      ...
      for (let j = 0; j <= radial; j++) {
        ...
        const nrm = norm([ring.n[0] * cos * ring.sy + ring.b[0] * sin * ring.sx, ring.n[1] * cos * ring.sy + ring.b[1] * sin * ring.sx, ring.n[2] * cos * ring.sy + ring.b[2] * sin * ring.sx]);
    ...
    for (let i = 0; i < total - 1; i++) {
      for (let j = 0; j < radial; j++) {
        ...
        const b = a + 1;
        const c = a + stride;
        const d = c + 1; // 绕序说明：binormal = cross(tangent, normal)，截面 θ 方向与 outward(normal) 相反，
  ...
  function smoothNormals(geo, precision = 4) {
    const map = new Map();
    const count = geo.positions.length / 3;

    const key = i => geo.positions[i * 3].toFixed(precision) + ',' + geo.positions[i * 3 + 1].toFixed(precision) + ',' + geo.positions[i * 3 + 2].toFixed(precision);
  ...
  function frontZ(profile, x, y, zScale = 1) {
  ...
  function spherePatch(dir, angleU, angleV, radii, segments = 26, rings = 5) {
    const w = norm(dir);
  ...
  function radialUV(geo) {
    ...
    for (let i = 0; i < count; i++) {
      uvs[i * 2] = Math.hypot(geo.positions[i * 3], geo.positions[i * 3 + 1], geo.positions[i * 3 + 2]) * inv;
    }
  ...
  function profileFrom(control, steps = 18) {
    if (control.length < 2) {
    ...
    for (let i = 0; i < control.length - 1; i++) {
      ...
      for (let s = 0; s <= n; s++) {
        ...
        const t2 = t * t;
        const t3 = t2 * t;
        const r = 0.5 * (2 * p1[0] + (-p0[0] + p2[0]) * t + (2 * p0[0] - 5 * p1[0] + 4 * p2[0] - p3[0]) * t2 + (-p0[0] + 3 * p1[0] - 3 * p2[0] + p3[0]) * t3);
