      ...
      _export("PetCatSprite", PetCatSprite = class PetCatSprite {
        ...
        load() {
          ...
          return _asyncToGenerator(function* () {
            var [body, tail, eyes] = yield Promise.all([_this.loadFrame('pets/layers/body'), _this.loadFrame('pets/layers/tail'), _this.loadFrame('pets/layers/eyes_closed')]);

            if (_this.disposed) {
              return;
            }

            _this.apply(_this.body, body);

            _this.apply(_this.tail, tail);

            _this.apply(_this.eyesClosed, eyes);
          })();
        }
        ...
        startBreath() {
          var k = BREATH.scaleY;
          var baseY = this.groundY + FOOT_OFFSET * this.s; // 缩放围绕节点原点（画布中心）进行，脚底会因此位移，必须反向补偿

          var lift = CONTENT_H * this.s / 2 * (k - 1);
        ...
        happyJump() {
          var baseY = this.groundY + FOOT_OFFSET * this.s;
          var s = this.s;
        ...
        dispose() {
          ...
          for (var node of [this.visual, this.tailPivot, this.eyesClosed]) {
            tween(node).stop();
          }
        ...
        spriteNode(parent, name, anchor) {
          var node = this.uiNode(parent, name);
          var sprite = node.addComponent(Sprite); // SIMPLE + 关闭 trim：整图按 1024×1024 原样绘制，不做九宫/裁剪
        ...
        loadFrame(path) {
          return new Promise((resolve, reject) => {
            resources.load("".concat(path, "/spriteFrame"), SpriteFrame, (err, frame) => {
              if (err || !frame) {
                reject(err || new Error("missing spriteFrame: ".concat(path)));
                return;
              }
